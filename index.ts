...
class BFRANode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BFRANode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBFRANode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : BFRANode {
        var curr : BFRANode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class BiFillRightAngle {

    curr : BFRANode = new BFRANode(0)
    dir : number = 1
